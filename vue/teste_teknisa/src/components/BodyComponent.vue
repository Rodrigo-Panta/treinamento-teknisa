    ...
    <div class="body">
      ...
      <div class="list">
        <div v-for="dev in devs" :key="dev.key" @click="openModal(dev)" class="card">
          ...
          <div class="cardInfo">
            <div>
              <img  class="imgLanguage" v-for="lang in dev.programmingLanguages" :key="lang.key" :src="imgLang[lang]" alt="">
            </div>
...
export default {
  ...
  methods: {
    openModal : function(dev) {
      ...
      let devLangs = '<div style="display:flex;flex-direction:row;" >';
      for(let i=0; i<devLength; i++) {
        devLangs += '<div><div style="padding:2%"><img style="height:30px;" src="'+ this.imgLang[dev.programmingLanguages[i]]+ '" /></div></div>' 
      }
      devLangs+='</div>';
      let modalStyle = "display:flex;flex-direction:row;";
      let modalImageStyle = "padding:16px";
      let xButtonStyle = "height:20px; margin:16px; position:absolute;"+
                          "right:0; background:none;border:none;font-weight:bold;font-size:15px;"
          ...
          template: `
            <div style="${modalStyle}">
              <div style="${modalImageStyle}">
                <img style="height:120px;" src="${dev.picture}" />
              </div>
              <div>
                <h4>${dev.name}</h4>
                <p>${dev.age} anos</p>
              ...
              <button style="${xButtonStyle}" @click="$emit('close')" >X</button>
            </div>
          `,
