...
function getCountVisibleCards() {
    return Array.from(
        document.getElementsByClassName("card")).filter(
            (card) =>   !card.getElementsByClassName.display  || 
...
function filter() {
    ...
    let interval = setInterval((_)=>{
        ...
        if(containerEl && containerEl.children && !changedText) {
            let visibleCard = updateVisibleCards(containerEl, search, operation, languages);
            updateResults(visibleCard);
        }
    }, 1000);
}
...
function updateVisibleCards(containerEl, search, operation, selectedLanguages) {
    ...
    Array.from(containerEl.children).forEach((cardEl)=>{
        ...
        if(titleEl){
            let isMatchName = matchesByName(titleEl.textContent, search);
            if(!isMatchName && operation=="AND") hideCard(cardEl);
            else if(isMatchName && operation=="OR") {
                showCard(cardEl);
                visibleCards++;
            } else if(isMatchName && operation=="AND") {
                let isMatchLanguage = matchesByLanguage(cardLanguages, selectedLanguages);
                if(isMatchLanguage) {
                    showCard(cardEl);
                    visibleCards++;
                } else {
...
function matchesByName(text1, text2){
    return text1.toLowerCase().includes(text2.toLowerCase());
}

function matchesByLanguage(cardLanguages, selectedLanguages){
...
function getUserInfo(id) {
    ...
    let userData = {};
    if(cardUser) {
        userData = ['age', 'mail', 'phone', 'github', 'username', 'description', 'descriptionAbility'].reduce(
            (acc, name) => {
        ...
        userData.languages = Array.from(cardUser.querySelectorAll(".languages > .iconLanguage")).reduce(
            function (acc, el) {
                acc[el.name] = `${el.getAttribute('experience')}`;
                return acc;
            }, {}
        )
...
function fillModal(userInfo){
    ...
    languages = Object.keys(userInfo.languages).map(
        (langCode) => {
            return `
        <div class="languageDescription">
            <div class="language-name">
                <img name="${langCode}" class="iconLanguage" 
                        src="images/${langCode}.png" alt="language"/>
